...
Oltre alle LSTM tradizionali abbiamo anche le \textbf{BLSTM} (Bidirectional LSTM) che sono
...
La \textbf{Precision} è il rapporto tra il numero di True Positive 
...
\begin{itemize}
    ...
    Di default è 100. Si è provato ad aumentare il valore ma a parte un maggiore
    tempo di addestramento, non si è notato un miglioramento significativo.
...
A parte per il random\_state, gli altri parametri sono stati lasciati
con i valori di default poiché, dopo vari test non sono stati riscontrati miglioramenti.
Per le features, si è notato che più sono le features, migliore è il risultato del modello.
...
Random Forest possiede un metodo chiamato \texttt{feature\_importances\_}
che permette di calcolare l'importanza di ogni feature nel modello.
...
Dopo vari test si è raggiunta un accuratezza del 91\% ma
...
Dalla Confusion Matrix \ref{fig:RF confusion matrix} si può vedere che il modello ha classificato
...
Dalla confusion matrix \ref{fig:LSTM confusion matrix} si può vedere che il modello ha classificato quasi tutti 
...
Per quanto riguarda la confusion matrix \ref{fig:BLSTM confusion matrix}, il modello ha classificato
